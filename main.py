...
class NeuralNetwork:
    ...
    def __init__(self, X: np.ndarray = None, Y: np.ndarray = None, example_set: np.ndarray = None, **kwargs) -> None:
        """
        initialize the Neural network requirements
        :param X: input set
        :param Y: output set
        :param example_set: if three_sets function has been used
        you can use this as your dataset insertion method
        :param kwargs: epsilon indicates the random initialization of theta matrix
        and lambda_ is for regularization respectively set on default of 1 and 0
        """
        self.example_set = example_set
        if self.example_set is not None:
            self.X = self.example_set[:, 0]
            self.Y = self.example_set[:, 1]
        else:
            self.Y = Y
            self.X = X
        ...
        self.kwargs = kwargs
        self.epsilon = _ if (_ := self.kwargs.get("epsilon")) is not None else 1
        self.lambda_ = _ if (_ := self.kwargs.get("lambda_")) is not None else 0
    ...
    @staticmethod
    def Three_sets(X: np.ndarray = None, Y: np.ndarray = None,
                   shuffle_on: bool = False, set_division: tuple = (60, 20, 20)) -> tuple:
        """
        divide the example set to 3 sets
        :param X: inputs
        :param Y: outputs
        :param shuffle_on: shuffles the set if it's been set on True
        :param set_division: the division ratio of each set (train, cv, test)
        :return: a tuple with 0th index train 1st index cv and 2nd index test
        """
        full_set = [(X[index], Y[index]) for index in range(len(X))]
        if shuffle_on:
            random.shuffle(full_set)
        No_examples = len(full_set)
        train = round(No_examples * (set_division[0] / 100))
        cv = round(No_examples * (set_division[1] / 100))
        test = round(No_examples * (set_division[2] / 100))
        if train + cv + test + 1 == No_examples:
            train += 1
        training_set = np.array(full_set[:train])
        cross_validation_set = np.array(full_set[train:train + cv])
        test_set = np.array(full_set[train + cv:])
    ...
    def Theta_matrix_creator(self) -> None:
        ...
        ** must be called just and only ONCE **
        ...
        for index, layer in enumerate(self.layers):
            ...
            self.Matrix_Theta[name] = (np.random.random([C, P]) * (self.epsilon * 2) - self.epsilon)

    def DELTA_matrix_creator(self, Matrix_name: dict) -> None:
        """
        partial derivative of cost function
        :param Matrix_name: input matrix
        :return: None
        """
        for index, layer in enumerate(self.layers):
            if index == 0:
            ...
            C = layer[1][1] + 1
            P = self.layers[index - 1][1][1] + 1
            Matrix_name[name] = np.zeros([C, P])

    @staticmethod
    def Z(layer: int, matrix_t: np.array, matrix_a: np.array) -> np.array:
        """
    ...
    def logistic(z: np.ndarray):
        ...
        :param z: pre-activation of a neuron
    ...
    def A1_matrix_creator(self, Xi: np.array, matrix_a: np.array) -> None:
        ...
        matrix_a["A_l1"] = np.ones([input_neurons + 1, 1])
    ...
    def forward_propagation(self, Xi: np.array, **kwargs) -> np.array:
        ...
        if kwargs.get("epsilon") is not None:
            ...
            for layer in range(2, len(self.layers) + 1):
                activation = NeuralNetwork.logistic(NeuralNetwork.Z(layer, self.Matrix_Theta_for_gradient_checking,
                                                                    self.Matrix_A_for_gradient_checking))
                self.Matrix_A_for_gradient_checking[f"A_l{layer}"] = activation
            ...
            for layer in range(2, len(self.layers) + 1):
                activation = NeuralNetwork.logistic(NeuralNetwork.Z(layer, self.Matrix_Theta, self.Matrix_A))
                self.Matrix_A[f"A_l{layer}"] = activation
                self.Matrix_A[f"A_l{layer}"][0] = 1
            return self.Matrix_A[f"A_l{len(self.layers)}"]

    def predict(self, Xi: np.array or int or float) -> np.array:
        """
        predicts as it has been named to do so :)
        :param Xi: one input feature vector
        :return: a np. array
        """
        ...
        for layer in range(2, len(self.layers) + 1):
            ...
            self.Matrix_A[f"A_l{layer}"] = activation
            self.Matrix_A[f"A_l{layer}"][0] = 1
        return self.Matrix_A[f"A_l{len(self.layers)}"][1:]

    def error(self, Yi: np.array) -> np.array:
        """
        returns the error of input calculated by hypothesis function
        :param Yi: output
        :return: error
        """
        error = self.Matrix_A[f"A_l{len(self.layers)}"] - np.reshape(np.insert(Yi, 0, 1), [self.k + 1, 1])
        self.Matrix_delta[f"d_l{len(self.layers)}"] = error
        return error

    def J_of_theta_for_one_example(self, Yi: np.array) -> None:
        """
    ...
    def back_propagation(self) -> None:
        ...
        for layer in reversed(range(2, len(self.layers))):
            ...
            self.Matrix_delta[f"d_l{layer}"] = delta * a
    ...
    def Run_one_time_through_examples(self) -> dict:
        ...
        for index, example in enumerate(self.X):
            ...
            self.J_of_theta_for_one_example(self.Y[index])
    ...
    def Gradiant(self) -> None:
        for layer in self.Matrix_DELTA:
            for j in range(self.Matrix_DELTA[layer].shape[1]):
                if j == 0:
                    self.Matrix_DELTA[layer][:, j] /= self.m
                else:
    ...
    def Train(self, learning_rate: float = 0.01,
              epochs=-1, gradiant_checking=False,
              epsilon=(10 ** -4), use_gradiant_of_GC=False,
              detailed=True):
        ...
        while epochs < end:
            ...
            self.Gradiant_decent(learning_rate, use_gradiant_of_GC=use_gradiant_of_GC)
...
my_x = np.array(np.random.random([2, 5]))
my_y = np.array(np.random.random([2, 1]))
my_set = NeuralNetwork.Three_sets(my_x, my_y)

ai = NeuralNetwork(example_set=my_set[0], epsilon=0.00001)
ai.add_layer(100)
ai.Train(epochs=200, detailed=False)
print("must be: 1\n", ai.predict(np.random.random([1, 5])[0]))
ai.plot_J()
